    ...
    public partial class Form1 : Form
    {
        ...
        private void MainGameTimerEvent(object sender, EventArgs e)
        {
            ...
            foreach (Control x in this.Controls)
            {
                ...
                {
                    x.Left -= this.obstacleSpeed;

                    if ( x.Left < -100 )
                    {
                        x.Left = this.ClientSize.Width + this.random.Next(200,500) + (x.Width * 15);
                        this.score++;
                    }

                    if ( this.trex.Bounds.IntersectsWith(x.Bounds) )
                    {
                        this.gameTimer.Stop();
                        this.trex.Image = Properties.Resources.dead;
                        this.txtScore.Text += " Press R to restart the game!";
